...
if (isset($_SESSION["id"]) && !empty($_SESSION["id"]) && isset($_SESSION["idCompany"]) && !empty($_SESSION["idCompany"])) {
    try {
        ...
        $response = $saldosController->getFacturaByFolio($folio);

        if($response->status==0)
        {
            $year=$response->factura["year"];

            $borarFactura=$saldosController->deleteFacturaByFolio($folio);

            $saldosController2 = new SaldosController($saldosModel,$year);
            $statusFactura=$saldosController2->setStatusFactura($folio,3);
            if($borarFactura==true && $statusFactura==true)
