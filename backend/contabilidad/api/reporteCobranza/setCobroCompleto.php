...
if (isset($_SESSION["id"]) && !empty($_SESSION["id"]) && isset($_SESSION["idCompany"]) && !empty($_SESSION["idCompany"])) {
    try {
        ...
        $responseF = $reporteCobranzaController->setFacturaCompleta($folio);
        $responseC = $reporteCobranzaController->setCobrosCompletos($ids);

        if($responseC==true && $responseF==true)
        {
            $response->setStatus(0);
            $response->setMessage("Se marco como completo correctamente");
        }
        else
        {
            $response->setStatus(1);
            $response->setMessage("Hubo un error al marcar como completo");
        }
