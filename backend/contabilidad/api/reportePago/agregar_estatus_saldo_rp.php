...
if (isset($_SESSION["id"]) && !empty($_SESSION["id"]) && isset($_SESSION["idCompany"]) && !empty($_SESSION["idCompany"])) {
    try {
        ...
        $res = $saldosController->agregarStatusSaldoRp($uuid, $status);

        if($res)
        {
            $response->setStatus(0);
            $message = "OK|UPDATED ESTATUS SALDO RP|"."id:".$uuid." |INSERTION OK";
        ...
        else
        {
            ...
            $message = "ERROR|UPDATED ESTATUS SALDO|"."id:".$uuid." |INSERTION FAIL";
            $response->message=$message;
